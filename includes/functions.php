 ...
 */
if ( ! function_exists( 'rc_include_folder' ) ) {
	function rc_include_folder( $path, $prefix = '', $ext = array( 'php' ) ) {
...
if ( ! function_exists( 'rch_get_placeholder_image' ) ) {
	function rch_get_placeholder_image() {
		$placeholder_image = get_option( 'rch_woocommerce_placeholder_img_src', 0 );
		...
		$source     = RC_MIGRATE_SHOPIFY_TO_WORDPRESS_IMAGES_DIR . 'rch-placeholder.jpg';
		$filename   = $upload_dir['basedir'] . '/rch-placeholder.png';
		...
		if ( ! file_exists( $filename ) ) {
			update_option( 'rch_woocommerce_placeholder_img_src', 0 );
		...
		update_option( 'rch_woocommerce_placeholder_img_src', $attach_id );
...
if ( ! function_exists( 'CHECK_PHP_INI_SETTINGS' ) ) {
	function CHECK_PHP_INI_SETTINGS() {
...
if ( ! function_exists( 'rc_rch_set_time_limit' ) ) {
	function rc_rch_set_time_limit( $limit = 0 ) {
		if ( function_exists( 'set_time_limit' ) && false === strpos( ini_get( 'disable_functions' ), 'set_time_limit' ) && ! ini_get( 'safe_mode' ) ) { // phpcs:ignore PHPCompatibility.IniDirectives.RemovedIniDirectives.safe_modeDeprecatedRemoved
			@set_time_limit( $limit ); // @codingStandardsIgnoreLine
		}
	}
}
if ( ! function_exists( 'rch_upload_image' ) ) {
	function rch_upload_image( $url, $post_parent = 0, $exclude = array(), $post_title = '', $desc = null ) {
